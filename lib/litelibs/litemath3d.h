...
lquat_t lquat_morm(const lquat_t* q);
...
lvec3_t lquat_mulvec3(const lquat_t* q, const lvec3_t* v)
{
  lquat_t vq, rq, conj, mul;

  vq = lquat(0, v->x, v->y, v->z);
  conj = lquat_conj(q);
  mul = lquat_mul(q, &vq);
  rq = lquat_mul(&mul, &conj);
...
lquat_t lquat_norm(const lquat_t* q)
{
  ...
  rq = *q;
  mag2 = rq.x*rq.x + rq.y*rq.y + rq.z*rq.z + rq.w*rq.w;
...
lquat_t lquat_lerp(const lquat_t* a, const lquat_t* b, float t)
{
  lquat_t sum, mula, mulb;

  mula = lquat_mulf(a, 1-t);
  mulb = lquat_mulf(b, t);
  sum = lquat_add(&mula, &mulb);
  return lquat_norm(&sum);
}
...
lquat_t lquat_slerp(const lquat_t* a, const lquat_t* b, float t)
{
  ...
  {
    float angle;
    lquat_t mula, mulb, div;

    angle = acos(dot);
    mula = lquat_mulf(&q, sin(angle*t));
    div = lquat_divf(&mula, sin(angle));
    mulb = lquat_mulf(a, sin(angle*(1-t)));
    return lquat_add(&mulb, &div);
  }
  else
  {
...
lmat4_t lmat4_lookatlh(const lvec3_t* center, const lvec3_t* eye, const lvec3_t* up)
{
  ...
  mat.m[9] = y.z;
  mat.m[10] = z.z;
  mul = lvec3_mulf(center, -1);
  return lmat4_translate(&mat, &mul);
}
